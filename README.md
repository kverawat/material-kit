...
2. ปัญหาของรูปภาพ image ที่อยู่ใน carousel มันไม่ยืดหยุ่นในการแก้ไข ต้นฉบับของต้นฉบับคือ Material Design Bootstrap (MDB) เขาอ้างอิง image foler ที่ ../../img/ ทำให้ Creativ-tim ก็ต้องเอาตามนั้นด้วย แต่ถ้าเราเปลี่ยน image folder เป็น ../../images/ เราก็ต้องมาตามไล่แก้ไข ซึ่งจริง ๆ ควรจะแก้ที่เดียวจบ ดังนั้นสิ่งที่จะต้องไปแก้ไขคือ `assets/scss/material-kit/variables/_carousel.scss` โดยเปลี่ยนเป็นการอ้างอิง `"../../img/arrow-left.png"` และ `"../../img/arrow-right.png"` เป็น `$mdb-img-path + "/arrow-left.png""` และ `$mdb-img-path + "/arrow-right.png"` แทน และให้ไปแก้ `assets/scss/material-kit/variables/_bootstrap-material-design-base.scss` โดยเพิ่ม `$mdb-img-path: '../../img' !default;` เข้าไป เพื่อบอกว่านี่คือตัวแปรที่เราจะใช้อ้างอิง image path ถ้าอยากเปลี่ยนก็มาเปลี่ยนที่นี่แทนที่เดียวจบ คือแก้ที่ `$mdb-img-path` และในอนาคตยังเอาไปใช้งานที่อื่น ๆ ในวันหลังได้ด้วย อันนี้เป็นเครดิตจาก [Github.com/Lumrenion/material-kit](https://github.com/Lumrenion/material-kit)
3. ปัญหาของสีพื้นหลังที่แก้ไขไม่ได้ ก็เดิม ๆ ครับ ไม่พูดซ้ำแล้ว ไปเพิ่ม **!default** ให้มันซะ โดยไปที่ `assets/scss/material-kit/variables/_bootstrap-material-design.scss` แล้วเพิ่ม !default ให้กับ $bg-primary, $bg-danger, $bg-warning, $bg-info, $bg-warning, $bg-rose, $bg-success, $bg-dark แค่นั้นเองครับ อันนี้เป็นเครดิตจาก [Github.com/Lumrenion/material-kit](https://github.com/Lumrenion/material-kit)
4.  ปัญหาของ card header ที่เปลี่ยนสีพื้นหลัง โดยไปที่ `assets/scss/material-kit/variables/_bootstrap-material-design.scss` แล้วเพิ่ม `$card-header-xxxxx-bg-from` `$card-header-xxxxx-bg-to` ให้ครบทุกสีหลักที่เป็นพื้นหลังที่ต้องการ จากนั้นก็ไปแก้อีกไฟล์คือที่ `assets/scss/material-kit/_cards.scss` โดยเปลี่ยนจากสีที่เคยกำหนดตรงตัวเช่น `background: linear-gradient(60deg, $purple-400, $purple-700);` ไปเป็น `background: linear-gradient(60deg, $card-header-primary-bg-from, $card-header-primary-bg-to);` ประมาณนี้ โดยทำให้ครบทุกสีหลักก็แค่นั้นครับ อันนี้เป็นเครดิตจาก [Github.com/Lumrenion/material-kit](https://github.com/Lumrenion/material-kit)
5. ปัญหาของ variable อื่น ๆ ที่พยายามจะแก้ไขแล้วทำไมมันไม่ได้เปลี่ยนตามที่เราแก้ ดูที่นี่ครับ [Missing !default in variables](https://github.com/creativetimofficial/ct-material-kit-pro/issues/101) **บอกใบ้ให้นิดนึง คือต้องใส่ !default เข้าไปครับ ถึงจะแก้แล้วเห็นผล ต้นฉบับที่ผม Forhed มานั้น บางตัวก็ไม่ได้ใส่ !default ครับ** ซึงผมยังไม่ได้แก้อะไรมากไปกว่าที่บอกไว้ด้านบน ถ้าแก้แล้วจะบอกครับ
